package  utils.md5
{
	...
	public class MD5 {
		...
		public static function hash(s:String) :String{
			...
			var digest:ByteArray = hashBinary(ba) ;	
			return IntUtil.toHex( digest.readInt()) + IntUtil.toHex( digest.readInt() ) + IntUtil.toHex( digest.readInt() ) + IntUtil.toHex( digest.readInt() );
		}
		
		public static function hashBytes(s:ByteArray) :String{	
			
			var digest:ByteArray = hashBinary(s) ;	
			return IntUtil.toHex( digest.readInt()) + IntUtil.toHex( digest.readInt() ) + IntUtil.toHex( digest.readInt() ) + IntUtil.toHex( digest.readInt() );			
		}
		
		public static function hashBytesByC(s:ByteArray) :ByteArray{	
			var digest:ByteArray = hashBinary(s);
			var md5:ByteArray = new ByteArray(); 			
			setIntTo4byte(md5,md5.position , digest.readInt());
			setIntTo4byte(md5,md5.position , digest.readInt());
			setIntTo4byte(md5,md5.position , digest.readInt());
			setIntTo4byte(md5,md5.position , digest.readInt());
			return md5;
		 ...
		 * @tiptext
		 */	 
		public static function hashBinary( s:ByteArray ):ByteArray {
		...
		private static function setIntTo4byte(bytes:ByteArray, offset:int, value:int):ByteArray
		{
